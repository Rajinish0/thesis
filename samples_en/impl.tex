...
However, vertex \textbf{A} is in multiple triangles. Figure~\ref{fig:normal_calc} shows my approach for calculating the normals. I all the 6 faces (triangles) that \textbf{A} is a part of. Let's call the neighboring vertices of A in the above picture $\mathbf{N_0}...\mathbf{N_5}$ in either counterclockwise or clockwise ordering (depending on which way defines the vectors in the up direction -- this needs to be checked empirically). Then the normal vector can be calculated as follows:
\[
